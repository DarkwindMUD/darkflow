...
async function validateRootBundle({ artifactDir, violations }) {
  ...
  const phase2EntryPath = path.join(artifactDir, "phase2", "index.html");
  if (!(await isFile(entryPath))) {
    return;
  }
  if (!(await isFile(phase2EntryPath))) {
    violations.push("missing required file: phase2/index.html");
    return;
  }

  const [html, phase2Html] = await Promise.all([
    fs.promises.readFile(entryPath, "utf8"),
    fs.promises.readFile(phase2EntryPath, "utf8"),
  ]);
  ...
  const rootBundleReferences = references.filter(
    (reference) =>
      /^assets\/phase2-[A-Za-z0-9_-]+\.js$/.test(reference),
  );
  const phase2BundleReferences = referencedJavaScriptPaths(phase2Html).filter(
    (reference) => /^assets\/phase2-[A-Za-z0-9_-]+\.js$/.test(reference),
  );

  if (rootBundleReferences.length !== 1) {
    violations.push(
  ...
  if (
    phase2BundleReferences.length !== 1 ||
    rootBundleReferences[0] !== phase2BundleReferences[0]
  ) {
    violations.push("index.html and phase2/index.html must reference the same generated application entry");
  }
  ...
  for (const reference of rootBundleReferences) {
    ...
    if (bundleContents.includes("/js/app.js")) {
      violations.push(`root bundle ${reference} contains legacy runtime handoff`);
    }
  }
}
