import {
  loadMapAreas as defaultLoadMapAreas,
  saveMapArea as defaultSaveMapArea,
  deleteMapArea as defaultDeleteMapArea,
  clearMapSource as defaultClearMapSource,
  pruneMapAreas as defaultPruneMapAreas,
} from './map-storage.js';
...
function createWorldState() {
  return {
    rooms: new Map(),
    areaVersions: new Map(),
    areaGenerations: new Map(),
    areaHydrationRevisions: new Map(),
  };
}

/** World-keyed shared graph/cache owner used by one or more session controllers. */
export function createMapDataV2WorldRepository() {
  const worlds = new Map();
  return {
    get(worldKey) {
      if (!worlds.has(worldKey)) worlds.set(worldKey, createWorldState());
      return worlds.get(worldKey);
    },
  };
}

/** Creates one session-owned MapData2 controller over an injected world repository. */
export function createMapDataV2(options = {}) {
  const gmcp = options.gmcp || { send: () => false };
  const loadMapAreas = options.loadMapAreas || defaultLoadMapAreas;
  const saveMapArea = options.saveMapArea || defaultSaveMapArea;
  const deleteMapArea = options.deleteMapArea || defaultDeleteMapArea;
  const clearMapSource = options.clearMapSource || defaultClearMapSource;
  const pruneMapAreas = options.pruneMapAreas || defaultPruneMapAreas;
  const worldRepository = options.worldRepository || createMapDataV2WorldRepository();
  let worldKey = options.worldKey || 'darkwind';
  let worldState = worldRepository.get(worldKey);

let rooms = worldState.rooms;
let currentRoomId = null;
let currentRoom = null;
let currentAreaName = '';
let areaVersions = worldState.areaVersions;
let areaGenerations = worldState.areaGenerations;
let areaHydrationRevisions = worldState.areaHydrationRevisions;
let mapEpoch = '';
let active = false;
...
function disposeMapDataLifecycle() {
  loadToken++;
...
function bindWorld(nextWorldKey) {
  worldKey = nextWorldKey;
  worldState = worldRepository.get(worldKey);
  rooms = worldState.rooms;
  areaVersions = worldState.areaVersions;
  areaGenerations = worldState.areaGenerations;
  areaHydrationRevisions = worldState.areaHydrationRevisions;
}

function resetSessionForWorld() {
  loadToken++;
  connectionGeneration++;
  clearCurrentRequest();
  clearCurrentRetry();
...
function configureWorld(identity = {}) {
  const next = safeSlug(
    identity.worldKey
      || [identity.host || 'darkwind', identity.port || ''].filter(Boolean).join('@'),
  );
  if (next === worldKey) return;
  flushPendingMapSave();
  bindWorld(next);
  resetSessionForWorld();
  return load();
}
...
function hasCurrentRoom() {
  return !!currentRoomId;
}

function hasPositionedCurrentRoom() {
...
function getRoom(id) {
  ...
  return normalizedId === currentRoomId && currentRoom
    ? currentRoom
...
function getAuthority() { return 'authoritative'; }
function getMapEpoch() { return mapEpoch; }

function canWalkExit(room, dir, destId) {
...
function mergeRoom(data, fallbackArea, preserveCurrentLive = true) {
  const next = normalizeRoomPayload(data, fallbackArea);
  if (!next) return 0;

  const old = rooms.get(next.id);
  const stored = old ? Object.assign(old, next) : next;
  rooms.set(next.id, stored);
  if (preserveCurrentLive && next.id === currentRoomId && currentRoom) {
    currentRoom = {
      ...stored,
      liveExits: { ...(currentRoom.liveExits || {}) },
      liveDoors: { ...(currentRoom.liveDoors || {}) },
      hasLiveObservation: !!currentRoom.hasLiveObservation,
    };
    stored.liveExits = {};
    stored.liveDoors = {};
    stored.hasLiveObservation = false;
  }
  return 1;
}
...
function processCurrent(data) {
  ...
  active = true;
  mergeRoom(data, data.area, false);
  currentRoomId = normalizeRoomId(data.id);
  const durableRoom = rooms.get(currentRoomId);
  currentRoom = durableRoom ? { ...durableRoom } : null;
  if (durableRoom) {
    durableRoom.liveExits = {};
    durableRoom.liveDoors = {};
    durableRoom.hasLiveObservation = false;
  }
...
function mergeServerAreaData(data) {
...
function mergeServerUpdate(data) {
...
function mergeServerUpdateV2(data) {
  ...
  if (liveCurrent && liveCurrent.area === data.area) {
    const durableCurrent = rooms.get(liveCurrent.id);
    currentRoom = durableCurrent ? {
      ...durableCurrent,
      liveExits: { ...(liveCurrent.liveExits || {}) },
      liveDoors: { ...(liveCurrent.liveDoors || {}) },
      hasLiveObservation: !!liveCurrent.hasLiveObservation,
      observedAt: liveCurrent.observedAt || durableCurrent.observedAt,
    } : liveCurrent;
  }
...
function requestAreaSync(area, forceFull) {
...
function processSyncError(data) {
  if (!data) return;
  if (Number(data.protocol) >= 2 || data.mapEpoch) protocol2Known = true;
...
function clearMapDataForArea(area, reset = {}) {
  if (!area) return;
...
function beginGlobalReset(reset = {}) {
  if (Number(reset.protocol) >= 2 || reset.mapEpoch) protocol2Known = true;
  cancelAllSyncs();
  loadToken++;
  clearCurrentRequest();
