...
function createWorldState() {
  return {
    rooms: new Map(),
    coordStatsByArea: new Map(),
    areaHydrationRevisions: new Map(),
  };
}

/** World-keyed shared learned-map graph/cache owner. */
export function createLearnedMapWorldRepository() {
...
/** Creates one session-owned Room.Info learned-map controller. */
export function createLearnedMap(options = {}) {
  ...
  const worldRepository = options.worldRepository || createLearnedMapWorldRepository();
  let worldKey = options.worldKey || DEFAULT_WORLD_KEY;
...
let saveTimer = null;
let lastRoomId = null;
let coordStatsByArea = worldState.coordStatsByArea;
...
function bindWorld(nextWorldKey) {
  ...
  coordStatsByArea = worldState.coordStatsByArea;
...
function configureWorld(identity = {}) {
  const nextKey = safeSlug(
    identity.worldKey
      || [identity.host || identity.name || 'world', identity.port || ''].filter(Boolean).join('@'),
  );
...
function processHello(data, connection = {}) {
  configureWorld({
    name: data && data.name,
...
function processRoomInfo(data) {
  ...
  const liveExits = { ...next.liveExits };
  const liveDoors = { ...next.liveDoors };
...
function clearMapDataForArea(area) {
  if (!area) return;
...
function getAuthority() { return 'learned'; }
...
function getStorageError() { return storageError; }

function getClearMapActionLabel() {
  return 'Clear';
}

function getClearMapActionTitle() {
